...
import { fetchParcelsInBounds, debounce, type MapBounds } from '../../services/gisService';
...
const RealMap = forwardRef<MapHandle, RealMapProps>(({
  ...
  // GIS viewport parcel state
  const [gisParcels, setGisParcels] = useState<CadastralPolygon[]>([]);
  ...
  // ── GIS Viewport Parcel Fetcher ───────────────────────────────────────────
  const fetchViewportParcels = useCallback(async (map: L.Map) => {
    const zoom = map.getZoom();
    if (zoom < 14) {
      // Below cadastral zoom: clear GIS parcels
      setGisParcels([]);
      return;
    }
    ...
    const bounds: MapBounds = {
      minLng: bnds.getWest(),
      minLat: bnds.getSouth(),
      maxLng: bnds.getEast(),
      maxLat: bnds.getNorth(),
    };

    setGisLoading(true);
    try {
      const fetched = await fetchParcelsInBounds(bounds, zoom);
      setGisParcels(fetched);
    } catch (e) {
      console.warn('[RealMap] GIS parcel fetch error', e);
    } finally {
      setGisLoading(false);
    }
  }, []);
  ...
  useEffect(() => {
    ...
    // Set up debounced GIS viewport parcel fetcher (300ms debounce)
    const debouncedFetch = debounce((m: L.Map) => fetchViewportParcels(m), 300);
    debouncedFetchRef.current = debouncedFetch as any;

    // Zoom + pan listeners → trigger GIS parcel fetch
    ...
    // Initial GIS fetch on mount
    fetchViewportParcels(map);
  ...
  useEffect(() => {
    ...
    if (!showCadastral || !activeLayers.parcels) {
      return;
    }

    // Merge GIS backend parcels with prop-based polygons (prop polygons take priority by ULPIN)
    const propULPINs = new Set((polygons || []).map(p => p.ulpin).filter(Boolean));
    const backendOnly = gisParcels.filter(gp => !propULPINs.has(gp.ulpin));
    const merged = [...(polygons || []), ...backendOnly];
    ...
    merged.forEach(poly => {
      // Guard against malformed polygon records without valid coordinates
      if (!poly || !poly.coordinates || !Array.isArray(poly.coordinates) || poly.coordinates.length < 3) {
        return;
      }
      ...
      const validCenter = poly.center && Array.isArray(poly.center) && poly.center.length >= 2 && !isNaN(poly.center[0]) && !isNaN(poly.center[1]);
      if (showLabels && validCenter) {
    ...
    polygonsGroupRef.current.bringToFront();
  }, [showCadastral, selectedSurveyNo, activeLayers, polygons, gisParcels, currentZoom, onSelectParcel]);
  ...
  return (
    <div className={`relative overflow-hidden ${className || 'w-full h-full min-h-[450px]'}`}>
      ...
      {/* GIS Loading Indicator */}
      {gisLoading && (
        <div className="absolute top-2 left-1/2 -translate-x-1/2 z-30 bg-[#125B50]/90 backdrop-blur-md text-white text-[10px] font-semibold px-3 py-1.5 rounded-full shadow-lg flex items-center gap-2 pointer-events-none">

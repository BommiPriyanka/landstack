...
// ── Default High-Fidelity Dataset (Matching Reference UI Designs) ───────────
export const LOCAL_PARCELS: ParcelRecord[] = [
  {
...
// Cadastral Survey Boundaries for Interactive Map Explorer (Centered on Ayigoundanpalayam)
...
export async function getVillageCadastralParcels(
  ...
  if (isSupabaseConfigured) {
    try {
      ...
      if (!error && data && data.length > 0) {
        const DELETED_SURVEYS = new Set(['125/4A', '125/4', '125/2', '124/2', '125/5']);
        const DELETED_ULPIN_PATTERNS = ['125-4A', '125-4', '125-2', '124-2', '125-5'];

        const mapped: CadastralPolygon[] = [];

        data.forEach((p: any, idx: number) => {
          if (
            DELETED_SURVEYS.has(p.survey_no) ||
            DELETED_ULPIN_PATTERNS.some(pat => (p.ulpin || '').includes(pat))
          ) {
            return;
          }
  ...
  // Fallback for Ayigoundanpalayam / Ayegoundanpalayam with active non-deleted parcels
  if (isAyigoundan) {
    return MOCK_CADASTRAL_POLYGONS;
  }
